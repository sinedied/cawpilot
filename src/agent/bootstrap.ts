...
function loadBootstrapPrompt(): string {
  ...
  const distPath = join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'templates', 'BOOTSTRAP.md');
 ...
 */
export async function runBootstrapStandalone(config: CawpilotConfig): Promise<void> {
