...
- **Web setup key** — generated in Bicep infra (`uniqueString`), passed as `CAWPILOT_WEBSETUP_KEY` env var, required in `X-Setup-Key` header for all `/api/setup/*` routes, validated with timing-safe comparison
...
- `CAWPILOT_WEBSETUP_KEY` — one-time setup wizard key (generated by Bicep)
...
When no workspace configuration is found, `cawpilot start` automatically triggers setup:
- If `CAWPILOT_WEBSETUP_KEY` env var is set → enters web setup mode
- Otherwise → runs interactive CLI setup, then starts normally

### Web Setup Mode

When `CAWPILOT_WEBSETUP_KEY` env var is present and either no config exists or `web.setupEnabled` is `true`, the `start` command enters **web setup mode** instead of normal operation:
